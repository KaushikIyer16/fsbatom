...
export default {
  ...
  initFileSystemResolver(dom){
    ...
    if (pathInput == null) {
      this.fileSystemResolver.setParseString("");
    }else{
  ...
  setDefaultRoot(){
    ...
    let currFileDirectory = atom.workspace.getActivePaneItem().buffer.file;
    if (currFileDirectory == null ) {
      rootInputText.setText("");
    }else{
      rootInputText.setText(resolveDirectory(currFileDirectory.path));
    }
